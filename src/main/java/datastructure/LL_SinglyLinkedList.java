...
//        nodeToDelete.next = null;
    ...
    public void recurInsert(int index, int val){
        head = recurInsert(index,val,head);
    }
    private Node recurInsert(int index, int val, Node node){
//        if(index==0){
...
//        }
//
//        recurInsert(--index,val,node.next);
        ...
        if (node == null) {
            throw new IndexOutOfBoundsException("Index out of bounds hhhh");
        }
        node.next = recurInsert(index - 1, val, node.next);
        return node;
    }
